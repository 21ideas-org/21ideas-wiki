...
- **Never scan wiki-en/ or wiki-ru/ directory trees to resolve wikilink targets.** Read `docs/link-map-en.md` or `docs/link-map-ru.md` instead — these are generated by `tools/build_link_map.py` and are always current.
...
docs/
  ...
  WIKI-SKILL.md             ← Wiki page generation: new ingest (Mode A) and update (Mode B)
  ENHANCE-SKILL.md          ← Single-page polish skill; replaces PAGE-ENHANCEMENT-STANDARD.md
...
tools/
  ...
  build_link_map.py     ← Generate docs/link-map-en.md and docs/link-map-ru.md
  check_parity.py       ← Check whether a slug exists in both wiki-en/ and wiki-ru/
```

To add new raw/ source files, see docs/INGEST-SKILL.md — that workflow is separate from the wiki ingest operation below.
For wiki page generation and updates, see docs/WIKI-SKILL.md.
For single-page polish, see docs/ENHANCE-SKILL.md.
...
Read `docs/ENHANCE-SKILL.md` in full before starting. After applying the standard:
