...
2. **Changed how the [[en/concepts/segwit|TXID]] is calculated** — the [[en/concepts/segwit|TXID]] now excludes witness data
...
SegWit fixed [[en/concepts/segwit|malleability]]: since signatures are excluded from the TXID calculation, changing the signature doesn't change the TXID.

### Problem 2: Block Capacity

SegWit introduced a new "[[en/concepts/segwit|block weight]]" metric:
...
**[[en/concepts/segwit|Bech32]] encoding:** Unlike legacy Base58 addresses, Bech32 has no uppercase letters, includes error detection, and is more compact. Addresses starting with `bc1` are native SegWit.

---

## The Blocksize War Context

SegWit was the solution to the 2015-2017 scaling debate. Large blockers wanted a [[en/concepts/governance|hard fork]] increasing block size; small blockers (and Bitcoin Core developers) proposed SegWit as a [[en/concepts/governance|soft fork]] that:
...
SegWit was the prerequisite for the Lightning Network. Without malleability fixed, Lightning's payment channel design couldn't be safely constructed. Lightning launched and scaled rapidly after SegWit activation in August 2017. See [[en/concepts/lightning-network]]].
...
[[en/glossary|Glossary]] | [[en/concepts/taproot|Taproot]] | [[en/concepts/address-types|address types]] | [[en/concepts/utxo|UTXO]] | [[en/concepts/lightning-network|Lightning Network]] | [[en/concepts/governance|soft fork]] | [[en/history/blocksize-war|blocksize war]] | [[en/books/blocksize-war|The Blocksize War]]

## Related Pages

- [[en/concepts/taproot]]] — the next major upgrade (builds on SegWit)
- [[en/concepts/address-types]]] — address types SegWit introduced
- [[en/concepts/utxo]]] — SegWit changes how UTXOs are structured
- [[en/concepts/lightning-network]]] — what SegWit enabled
- [[en/history/blocksize-war]]] — the political context of SegWit's activation
- [[en/concepts/governance]]] — UASF demonstrated user power
