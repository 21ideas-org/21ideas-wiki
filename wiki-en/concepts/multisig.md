...
See also: [[en/concepts/security]]], [[en/concepts/taproot]]], [[en/concepts/address-types]]], [[en/concepts/utxo]]], [[en/practice/storage]]], [[en/glossary]]]
...
The **2-of-3** setup is the sweet spot for individuals: one key can be lost or stolen without losing funds, but an attacker who steals one key still cannot spend. This is the setup recommended in [[en/practice/storage]]].
...
**P2WSH (Pay-to-Witness-Script-Hash)** — Native SegWit multisig. Script moved into the witness (lower fees via [[en/concepts/segwit]]]). Still reveals the multisig structure when spending.

**P2TR + MuSig2 (Taproot multisig)** — The modern standard. Using [[en/concepts/taproot]]] and the MuSig2 key-aggregation protocol, N-of-N multisig participants can combine their keys into a single aggregated public key that is indistinguishable from a single-sig key on-chain. The N-of-N structure is invisible to blockchain observers. This provides significant privacy benefits and reduces fees further.

---

## MuSig2 and Taproot: N-of-N Looks Like Single-Sig

[[en/concepts/taproot]]] introduced Schnorr signatures, which are linearly aggregatable — multiple signatures can be combined into one. The MuSig2 protocol exploits this:
...
**Sparrow Wallet** — Desktop wallet with full multisig support. Can coordinate signing with multiple hardware wallets, display PSBT workflow, and connect to the user's own node for privacy (see [[en/practice/privacy-practice]]]).
...
- [[en/concepts/security]]] — broader security model including seed phrases, hardware wallets, and threat modeling
- [[en/concepts/taproot]]] — Schnorr signatures and MuSig2 foundation
- [[en/concepts/address-types]]] — P2SH, P2WSH, P2TR address formats
- [[en/concepts/utxo]]] — UTXO model underlying all Bitcoin transactions
- [[en/practice/storage]]] — practical guide to cold storage and multisig setups
- [[en/glossary]]] — term definitions including PSBT, output descriptor, quorum
