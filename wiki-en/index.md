---
title: "Wiki Index"
category: index
tags: [bitcoin, wiki, index, navigation]
language: en
source: "Synthesized from raw/ sources + glossary"
updated: "2026-04-07"
quality: reference
sources: []
synthesized_date: "2026-04-07"
completeness: high
---

# Wiki Index

Bitcoin knowledge base built from 21ideas.org — a Russian-language Bitcoin education site (~308 source files). All source material is in Russian; wiki synthesis is in English.

---

## Concepts

- [Bitcoin](wiki-en/concepts/bitcoin.md) — What Bitcoin is, why it matters, core properties
- [Money](wiki-en/concepts/money.md) — What money is, monetary history, sound money
- [Proof of Work](wiki-en/concepts/proof-of-work.md) — The consensus mechanism that secures Bitcoin
- [The Lightning Network](wiki-en/concepts/lightning-network.md) — Layer 2 payment channels
- [UTXOs](wiki-en/concepts/utxo.md) — Bitcoin's accounting model
- [Privacy](wiki-en/concepts/privacy.md) — Bitcoin privacy: threats, tools, techniques
- [Security](wiki-en/concepts/security.md) — Self-custody, seed phrases, multisig, hardware wallets
- [Multisig (Multisignature)](wiki-en/concepts/multisig.md) — M-of-N schemes, PSBT workflow, collaborative custody, MuSig2
- [Scarcity](wiki-en/concepts/scarcity.md) — 21 million cap, halving, digital scarcity
- [Bitcoin Governance](wiki-en/concepts/governance.md) — Who controls Bitcoin, how changes happen
- [Mining](wiki-en/concepts/mining.md) — SHA-256, proof of work, difficulty adjustment, ASICs, block rewards
- [SegWit](wiki-en/concepts/segwit.md) — 2017 soft fork: malleability fix, block weight, witness data
- [Taproot](wiki-en/concepts/taproot.md) — 2021 soft fork: Schnorr signatures, MAST, P2TR, MuSig2
- [Address Types](wiki-en/concepts/address-types.md) — P2PK, P2PKH, P2SH, P2WPKH, P2WSH, P2TR explained
- [Bitcoin Protocol Stack](wiki-en/concepts/protocol-stack.md) — Layered architecture: P2P, base chain, Script, Lightning, apps
- [Hashcash](wiki-en/concepts/hashcash.md) — Adam Back's 1997 PoW anti-spam system; direct precursor to Bitcoin mining
- [Cantillon Effect](wiki-en/concepts/cantillon-effect.md) — Non-neutral distributional effect of money creation; why Bitcoin is Cantillon-resistant
- [Blockchain](wiki-en/concepts/blockchain.md) — Chain of blocks, PoW linkage, tamper-evident ledger (Bitcoin-specific framing)
- [BIP](wiki-en/concepts/bip.md) — Bitcoin Improvement Proposals; upgrade design docs and activation context
- [Byzantine Generals Problem](wiki-en/concepts/byzantine-generals-problem.md) — Trust-minimized agreement; how sources map it to Bitcoin consensus
- [Decentralization](wiki-en/concepts/decentralization.md) — No central control; validation, nodes, and blocksize-war nuance
- [Deflation](wiki-en/concepts/deflation.md) — Purchasing-power framing vs fiat; disinflation vs popular “deflation” language
- [Difficulty Adjustment](wiki-en/concepts/difficulty-adjustment.md) — 2016-block retargeting; ~10-minute block schedule
- [Double Spend](wiki-en/concepts/double-spend.md) — The digital-cash problem Bitcoin solves; confirmations and 51% nuance
- [Forks](wiki-en/concepts/forks.md) — Accidental splits, soft vs hard forks, Bitcoin Cash example
- [Mempool](wiki-en/concepts/mempool.md) — Unconfirmed transactions; miners, fees, reorgs
- [Bitcoin Whitepaper](wiki-en/concepts/bitcoin-whitepaper.md) — 2008 technical paper; foundational but incomplete spec framing
- [AML (Anti-Money Laundering) in Bitcoin](wiki-en/concepts/aml.md) — 21ideas framing of AML as surveillance + custodial censorship risk
- [b-money](wiki-en/concepts/b-money.md) — Wei Dai’s 1998 proposal; “first draft” precursor in Genesis Files
- [Bit Gold](wiki-en/concepts/bit-gold.md) — Nick Szabo’s digital gold proposal; near-miss predecessor
- [RPOW](wiki-en/concepts/rpow.md) — Hal Finney’s reusable proof-of-work prototype (2004)

## Entities

- [Satoshi Nakamoto](wiki-en/entities/satoshi-nakamoto.md) — Anonymous creator of Bitcoin
- [Hal Finney](wiki-en/entities/hal-finney.md) — First Bitcoin recipient, cypherpunk legend
- [Nick Szabo](wiki-en/entities/nick-szabo.md) — Bit Gold, Shelling Out, social scalability
- [Cypherpunks](wiki-en/entities/cypherpunks.md) — The movement that made Bitcoin possible
- [Adam Back](wiki-en/entities/adam-back.md) — Hashcash creator; cited precursor to Bitcoin PoW
- [Tim May](wiki-en/entities/tim-may.md) — Crypto Anarchist Manifesto; cypherpunk co-founder framing
- [Eric Hughes](wiki-en/entities/eric-hughes.md) — Author of A Cypherpunk’s Manifesto (“cypherpunks write code”)
- [David Chaum](wiki-en/entities/david-chaum.md) — eCash / DigiCash; privacy-forward but centralized predecessor
- [Phil Zimmermann](wiki-en/entities/phil-zimmermann.md) — Creator of PGP; signatures and software authenticity mindset
- [Gigi (dergigi)](wiki-en/entities/gigi.md) — Bitcoin philosopher, author of 21 Ways
- [Parker Lewis](wiki-en/entities/parker-lewis.md) — Author of "Gradually, Then Suddenly"; Head of BD at Unchained Capital
- [Saifedean Ammous](wiki-en/entities/saifedean-ammous.md) — Author of *The Bitcoin Standard* and *The Fiat Standard*

## Books

- [Inventing Bitcoin](wiki-en/books/inventing-bitcoin.md) — Yan Pritzker — technical primer
- [Sovereignty Through Mathematics](wiki-en/books/sovereignty-through-mathematics.md) — Knut Svanholm
- [The Bullish Case for Bitcoin](wiki-en/books/bullish-case.md) — Vijay Boyapati
- [The Fiat Standard](wiki-en/books/fiat-standard.md) — Saifedean Ammous
- [The Price of Tomorrow](wiki-en/books/price-of-tomorrow.md) — Jeff Booth
- [The Sovereign Individual](wiki-en/books/sovereign-individual.md) — Davidson & Rees-Mogg (1997)
- [The Blocksize War](wiki-en/books/blocksize-war.md) — Jonathan Bier
- [21 Ways](wiki-en/books/21-ways.md) — Gigi (dergigi)

## Article Series

- [Gradually, Then Suddenly](wiki-en/series/gradually-then-suddenly.md) — Parker Lewis (17 parts)
- [Discovering Bitcoin](wiki-en/series/discovering-bitcoin.md) — Giacomo Zucco (7 parts)
- [Genesis Files](wiki-en/series/genesis-files.md) — Aaron van Wirdum — Bitcoin's intellectual prehistory
- [Bitcoin Astronomy](wiki-en/series/bitcoin-astronomy.md) — Dhruv Bansal — post-hyperbitcoinization
- [Silk Road](wiki-en/series/silk-road.md) — Bitcoin's first great marketplace
- [OXT Research](wiki-en/series/oxt-research.md) — Samourai Wallet: blockchain analysis and Bitcoin privacy (4 parts)
- [What I Learned From Bitcoin](wiki-en/series/what-i-learned-from-bitcoin.md) — Gigi: philosophical, economic, technological lessons

## History

- [Timeline](wiki-en/history/timeline.md) — Key events from 2008 to present
- [Pre-Bitcoin: The Cypherpunks](wiki-en/history/pre-bitcoin-cypherpunks.md) — eCash, Hashcash, Bit Gold
- [The Blocksize War](wiki-en/history/blocksize-war.md) — 2015–2017 scaling conflict

## Philosophy

- [Philosophy Overview](wiki-en/philosophy/overview.md) — 17 core philosophical themes

## Practice

- [Buying Bitcoin](wiki-en/practice/buying.md) — P2P, no-KYC options
- [Storage & Self-Custody](wiki-en/practice/storage.md) — Hardware wallets, multisig, cold storage
- [Lightning Tools](wiki-en/practice/lightning-tools.md) — Wallets, apps, services
- [Privacy in Practice](wiki-en/practice/privacy-practice.md) — CoinJoin, Dojo, GrapheneOS
- [Running a Bitcoin Node](wiki-en/practice/running-a-node.md) — Why it matters, hardware, Bitcoin Core, Umbrel, connecting wallets

## Topics

- [Bitcoin for Dissidents](wiki-en/topics/bitcoin-dissidents.md) — Belarus, Nigeria, Hong Kong, Russia: Bitcoin under authoritarianism
- [Bitcoin's Seven Network Effects](wiki-en/topics/network-effects.md) — Trace Mayer's framework for Bitcoin's compounding adoption

## Glossary

- [Glossary](wiki-en/glossary.md) — All key Bitcoin terms: A–Z (Cyrillic) and Latin abbreviations (ASIC, BIP, HTLC, UTXO, etc.)

## Overview

- [[en/overview|Synthesis]] — Big-picture synthesis: what this knowledge base is about

---

## Sources

*Synthesized from multiple sources in the 21ideas.org raw/ library. No single canonical source article.*

---

*This wiki contains 54 pages synthesized from 308 source files in the 21ideas.org raw/ library. It is fully interlinked and ready for public hosting via Quartz or Obsidian Publish. Last updated: 2026-04-07.*

---

## Related Pages

- [[en/overview]]] — Big-picture synthesis of the wiki's content
- [[en/glossary]]] — Key Bitcoin terms A–Z
- [[en/concepts/bitcoin]]] — The core concept
- [[en/history/timeline]]] — Chronological history of Bitcoin
